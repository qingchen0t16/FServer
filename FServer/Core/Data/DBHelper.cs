...
namespace FServer.Core
{
    ...
    {
        // 单例
        public static readonly DBHelper I = new DBHelper();
        // 数据库池
        ...
        /// <returns></returns>
        public bool Connection(string ip,int port,string user,string passwd,string database,string charset, string dbName,  bool exThrow = false)
        {
            try
            {
                connList.Add(dbName, new MySqlConnection($"server={ip};port={port};user={user};password={passwd};database={database};Charset={charset};"));
                Logger.I.Info("连接数据库成功");
            }
            catch (Exception ex)
            {
                if (exThrow)
